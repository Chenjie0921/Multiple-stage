...
def get_train_utils(opt, model_parameters, parameters_feature, ratio):
    ...
    if opt.is_master_node:
        train_logger = Logger(opt.result_path / 'train.log',
                              ['epoch', 'loss_total', 'acc_current', 'acc_future', 'acc_ronghe' ,'lr'])
        acc_logger = Logger(opt.result_path / 'acc.log', ['epoch', 'acc_epoch_test'])
        train_batch_logger = Logger(
            opt.result_path / 'train_batch.log',
            ['epoch', 'batch', 'iter', 'loss_total', 'loss_current', 'loss_future', 'loss_feature', 'loss_ronghe',
             'loss_d', 'loss_g', 'acc_current', 'acc_future', 'acc_ronghe', 'lr'])
    else:
        train_logger = None
    ...
    optimizer_1 = SGD(parameters_feature,
                      lr=opt.learning_rate,
                      momentum=opt.momentum,
                      dampening=dampening,
                      weight_decay=opt.weight_decay,
...
def main_worker(index, opt):
    ...
    if not opt.no_train:
        (train_loader, train_sampler, train_logger, train_batch_logger,
         optimizer, optimizer_1, scheduler, scheduler_1, acc_logger) = get_train_utils(opt=opt,
    ...
    MLP = decoder_fuser(dim=2048, num_heads=8, num_layers=3)
    ...
    optimizer_2 = SGD([{'params': MLP.parameters()},
                       ...
                       {'params': classifer.parameters()}], lr=0.00005, momentum=0.9)
    # scheduler_3 = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer_2, mode='max')
    scheduler_3 = lr_scheduler.MultiStepLR(optimizer_2, [30, 50, 70], gamma=0.1)
    ...
    #     print(i)
    #     print(inputs.shape)# torch.Size([1, 3, 16, 112, 149])
    #     # print(inputs_90.shape)# torch.Size([10, 3, 16, 112, 149])
    #     print(targets)
    #     break
