...
def load_pretrained_model(model, pretrain_path, model_name, n_finetune_classes):
    if pretrain_path:
        ...
        if pretrain_path.name == 'resnext-101-kinetics-hmdb51_split3.pth':
            pass
            pretrain['state_dict'] = {'module.'+str(key): value for key, value in
                                      pretrain['state_dict'].items()}
            # pretrain['state_dict'] = {str(key).replace("module.", ""): value for key, value in
            #                           pretrain['state_dict'].items()}
